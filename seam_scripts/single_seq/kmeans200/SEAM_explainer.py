"""SEAM k-means clustering + MetaExplainer at 200 clusters for the 100K K562 lib.

Identical to seam_scripts/single_seq/kmeans100/SEAM_explainer.py but defaults to
200 k-means clusters and writes to a distinct output directory
(results/single_seq/foregrounds_kmeans200) so the 50- and 100-cluster
foregrounds are left in place. Reads the same shared, pre-merged ISM +
mutagenesis libraries (100K).

Usage:
    python SEAM_explainer.py [--seq-idx 3609] [--n-clusters 200]
"""
...
OUT_DIR = PROJ_ROOT / "results/single_seq/foregrounds_kmeans200"
