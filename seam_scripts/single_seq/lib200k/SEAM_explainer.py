"""SEAM k-means clustering + MetaExplainer for the 200K K562 lib -- 200K variant.

Identical to seam_scripts/single_seq/SEAM_explainer.py but reads the 200K ISM +
mutagenesis libraries and writes to results/single_seq/foregrounds_200k/.
Defaults to 100 k-means clusters. Uses ism_centered as the attribution map.

Usage:
    python SEAM_explainer.py [--seq-idx 3609] [--n-clusters 100]
"""
...
ISM_DIR = PROJ_ROOT / "results/single_seq/ism_200k"
OUT_DIR = PROJ_ROOT / "results/single_seq/foregrounds_200k"
...
def main():
    ...
    print(f"N={len(x_mut)}  L={x_mut.shape[1]}  k={N_CLUSTERS}  using ism_centered as attribution maps")
