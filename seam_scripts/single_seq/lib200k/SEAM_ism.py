"""ISM attribution maps for the 200K K562 mutagenesis library -- 200K variant.

Identical to seam_scripts/single_seq/SEAM_ism.py but reads the 200K mutagenesis
library and writes to results/single_seq/ism_200k/, leaving the 100K ISM in
place. For each base sequence the 200bp variable region [15:215] is scored by
saturation single-position mutagenesis (800 mutants/seq); ism_centered is
mean-centered across channels.

Output: results/single_seq/ism_200k/{SEQ_ID}.h5  (ism_raw, ism_centered, predictions)

Usage:
    python SEAM_ism.py [--seq-idx 3609] [--start 0 --end 200000] [--batch-size 1024]
"""
...
MUT_LIB_DIR = PROJ_ROOT / "results/single_seq/mutagenesis_lib_200k"
OUT_DIR = PROJ_ROOT / "results/single_seq/ism_200k"
